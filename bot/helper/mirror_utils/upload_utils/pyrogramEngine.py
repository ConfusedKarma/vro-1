...
from bot import DOWNLOAD_DIR, AS_DOCUMENT, AS_DOC_USERS, AS_MEDIA_USERS, CUSTOM_FILENAME, \
                 EXTENSION_FILTER, app, LOG_CHANNEL
...
class TgUploader:
    ...
    def __upload_file(self, up_path, file_, dirpath):
        if LOG_CHANNEL:
            set = LOG_CHANNEL.copy()
            setstr = str(set)[1:-1]
            LOG_CHANNEL = int(setstr)
            leechchat = LOG_CHANNEL
            ...
            if not self.__as_doc:
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES):
                    ...
                    if not file_.upper().endswith(("MKV", "MP4")):
                        ...
                        osrename(up_path, new_path)
                        up_path = new_path
                    self.__sent_msg = self.__sent_msg.reply_video(chat_id=leechchat, video=up_path,
                                                                  quote=True,
                                                                  caption=cap_mono,
                                                                  duration=duration,
                                                                  width=width,
                    ...
                    self.__sent_msg = self.__sent_msg.reply_audio(chat_id=leechchat, audio=up_path,
                                                                  quote=True,
                                                                  caption=cap_mono,
                                                                  duration=duration,
                                                                  performer=artist,
                    ...
                    self.__sent_msg = self.__sent_msg.reply_photo(chat_id=leechchat, photo=up_path,
                                                                  quote=True,
                                                                  caption=cap_mono,
                                                                  disable_notification=True,
                                                                  progress=self.__upload_progress)
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES) and thumb is None:
                    ...
                    if self.__is_cancelled:
                        if self.__thumb is None and thumb is not None and ospath.lexists(thumb):
                            osremove(thumb)
                        return
                self.__sent_msg = self.__sent_msg.reply_document(chat_id=leechchat, document=up_path,
                                                                  quote=True,
                                                                 thumb=thumb,
                                                                 caption=cap_mono,
                                                                 disable_notification=True,

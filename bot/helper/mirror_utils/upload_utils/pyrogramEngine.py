        ...
        try:
            if not self.__as_doc:
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES):
                    ...
                    self.__sent_msg = self.__sent_msg.send_audio(audio=up_path,
                                                                  quote=True,
                                                                  caption=cap_mono,
                                                                  duration=duration,
                                                                  performer=artist,
                    ...
                    self.__sent_msg = self.__sent_msg.send_photo(photo=up_path,
                                                                  quote=True,
                                                                  caption=cap_mono,
                                                                  disable_notification=True,
                                                                  progress=self.__upload_progress)
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES) and thumb is None:
                    ...
                    if self.__is_cancelled:
                        if self.__thumb is None and thumb is not None and ospath.lexists(thumb):
                            osremove(thumb)
                        return
                self.__sent_msg = self.__sent_msg.send_document(document=up_path,
                                                                 quote=True,
                                                                 thumb=thumb,
                                                                 caption=cap_mono,
                                                                 disable_notification=True,
